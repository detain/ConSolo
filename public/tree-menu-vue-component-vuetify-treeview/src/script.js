...
let gid = 1 // Sorting with gid
let kpdata = {} // The name corresponding to the record id
function modify(level,children) {
    return _.map(children, function (v) {
        gid += 1
        v.id = guid()
        v.gid = gid
        v.level = level
        ...
        kpdata[v.id] = _.cloneDeep(v)
        v.name=usename
        if (v.children) {
            v.children = modify(level+1,v.children)
        }
        return v
    })
}

// Add id and change display name, only id can be selected
data.id = guid()
data.gid = 1
data.level = 1
kpdata[data.id] = _.cloneDeep(data) 
data.children = modify(2,data.children)
...
new Vue({
    ...
    computed: {
        getss: function () {
            //console.log('computed getss')
            let vo = this
            //ss
            let ss = _.map(vo.selects, function (k) {
                return kpdata[k]
            })            
            //sortBy
            ss = _.sortBy(ss,'gid')          
            //console.log('getss ss',oo(ss))
            return ss
        }
    },
    methods: {
        remove: function (s) {
            //console.log('methods remove',s)
            // If you click on the parent layer to select all sublayers, and then use chip close to remove the single sublayer,
            // Because the state of the parent layer cannot be changed first, it will be forced to be restored by v-treeview to check all sublayers
            // If chip close removes the parent layer, it will also be restored because it cannot drive v-treeview to clear all sublayers
            // Only by manually selecting multiple sub-layers, can we use chip close to remove

            let vo = this

            let ss = _.cloneDeep(vo.selects)
            _.pull(ss, s.id)
            vo.selects = ss //Only memory changes can be triggered computed getss
            //console.log('remove ss',oo(ss))
