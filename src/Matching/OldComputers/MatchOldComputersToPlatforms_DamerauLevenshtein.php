...
foreach ($platformNames as $name) {
    ...
    $results = [];
    foreach ($name2id as $key => $id) {
        $DamerauLevenshtein = new DamerauLevenshtein($name, $key);
        $similarity = $DamerauLevenshtein->getSimilarity();
        if (!array_key_exists($similarity, $results))
            $results[$similarity] = [];
    ...
    foreach ($values as $value) {
        $idx++;
        if (count($results[$value]) >= 5 ||$idx >= 5)
            break;
        echo $value.': '.implode(', ', $results[$value]).','.PHP_EOL;
        foreach ($results[$value] as $result)
            $outResults[] = [$result, $value];
          
    }
    $jsonOut[] = '"'.$name.'": '.json_encode($outResults);
}
$json = '{'.PHP_EOL.'    '.implode(','.PHP_EOL.'    ', $jsonOut).PHP_EOL.'}';
echo $json.PHP_EOL;
file_put_contents('ocplatforms_dameraulevenshtein.json', $json);
exit;
